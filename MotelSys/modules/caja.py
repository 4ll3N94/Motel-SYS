...
Diseño: Dark Luxury & Enterprise Grade Financial Suite (Latin-1 Safe & 5 KPIs)
===============================================================================
"""
...
def _limpiar_texto_pdf(texto: str) -> str:
    """Elimina caracteres fuera del rango latin-1 para evitar UnicodeEncodeError."""
    if not texto:
        return ""
    reemplazos = {
        "\u2022": "-",
...
class PDFCierreContable(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 5, _limpiar_texto_pdf("INVERSIONES SAIBABA C.A."), 0, 1, "C")
        self.set_font("Helvetica", "B", 8.5)
        self.cell(0, 4, _limpiar_texto_pdf("RIF: J-30250227-6 - MOTEL EL EDEN"), 0, 1, "C")
        self.set_font("Helvetica", "B", 10.5)
        self.cell(0, 5, _limpiar_texto_pdf("COMPROBANTE DETALLADO DE CIERRE DE CAJA Y ARQUEO"), 0, 1, "C")
    ...
    def footer(self):
        ...
        self.cell(0, 8, _limpiar_texto_pdf(f"Documento Contable y de Auditoria Interna - Pagina {self.page_no()}"), 0, 0, "C")
...
class FrameCaja(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        def crear_tarjeta_resumen(col, titulo, icono, tag_text, tag_color):
            ...
            ctk.CTkLabel(badge, text=tag_text, font=("Arial", 8, "bold"), text_color=tag_color).pack(padx=4, pady=1)
            ...
            lbl_sub = ctk.CTkLabel(card, text="--", font=("Arial", 8), text_color=self.C_TEXT_MUTED)
    ...
    # =========================================================================
    # GENERADOR DEL REPORTE PDF (LATIN-1 SEGURO)
    # =========================================================================
    def exportar_pdf_cierre(self):
        ...
        pdf.cell(35, 5, _limpiar_texto_pdf("FECHA DE CIERRE:"), 0, 0, "L")
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(60, 5, _limpiar_texto_pdf(str(self.datos_turno['fecha'])), 0, 0, "L")

        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(20, 5, _limpiar_texto_pdf("TURNO:"), 0, 0, "L")
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(75, 5, _limpiar_texto_pdf(str(self.datos_turno['turno'])), 0, 1, "L")

        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(35, 5, _limpiar_texto_pdf("RECEPCIONISTA:"), 0, 0, "L")
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(60, 5, _limpiar_texto_pdf(str(self.datos_turno['recepcionista']).upper()), 0, 0, "L")

        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(20, 5, _limpiar_texto_pdf("TASA BCV:"), 0, 0, "L")
        pdf.set_font("Helvetica", "", 9)
        pdf.cell(75, 5, _limpiar_texto_pdf(f"{self.datos_turno['tasa_bcv']:.2f} Bs / USD"), 0, 1, "L")
        pdf.ln(4)

        # Cuadro 1: Habitaciones
        pdf.set_fill_color(220, 220, 220)
        pdf.set_font("Helvetica", "B", 8)
        pdf.cell(28, 6, _limpiar_texto_pdf("HABITACION"), 1, 0, "C", fill=True)
        pdf.cell(14, 6, _limpiar_texto_pdf("CANT"), 1, 0, "C", fill=True)
        pdf.cell(52, 6, _limpiar_texto_pdf("SUBTOTAL (ALQUILER)"), 1, 0, "C", fill=True)
        pdf.cell(42, 6, _limpiar_texto_pdf("MERCANCIA"), 1, 0, "C", fill=True)
        pdf.cell(30, 6, _limpiar_texto_pdf("TOTAL BS"), 1, 0, "C", fill=True)
        pdf.cell(24, 6, _limpiar_texto_pdf("TOTAL $"), 1, 0, "C", fill=True)
        pdf.ln()

        pdf.set_font("Helvetica", "", 7.5)
        for h in self.datos_turno["habitaciones"]:
            sub_alq_str = _limpiar_texto_pdf(self.formatear_metodos_str(h["alq_usd"], h["alq_bs"], h["alq_p2p"], h["alq_punto"]))
            sub_mer_str = _limpiar_texto_pdf(self.formatear_metodos_str(h["mer_usd"], h["mer_bs"], h["mer_p2p"], h["mer_punto"]))

            pdf.cell(28, 5.5, _limpiar_texto_pdf(str(h["habitacion"])[:16]), 1, 0, "C")
            pdf.cell(14, 5.5, str(h["cantidad"]), 1, 0, "C")
            pdf.cell(52, 5.5, sub_alq_str[:34], 1, 0, "C")
            pdf.cell(42, 5.5, sub_mer_str[:28], 1, 0, "C")
            pdf.cell(30, 5.5, _limpiar_texto_pdf(formatear_bs(h["total_bs"]).replace(" Bs", "")), 1, 0, "R")
        ...
        pdf.set_fill_color(220, 220, 220)
        pdf.set_font("Helvetica", "B", 8)
        pdf.cell(48, 6, _limpiar_texto_pdf("PRODUCTO"), 1, 0, "C", fill=True)
        pdf.cell(16, 6, _limpiar_texto_pdf("CANT"), 1, 0, "C", fill=True)
        pdf.cell(66, 6, _limpiar_texto_pdf("SUBTOTAL (METODOS)"), 1, 0, "C", fill=True)
        pdf.cell(34, 6, _limpiar_texto_pdf("TOTAL BS"), 1, 0, "C", fill=True)
        pdf.cell(26, 6, _limpiar_texto_pdf("TOTAL $"), 1, 0, "C", fill=True)
        pdf.ln()

        pdf.set_font("Helvetica", "", 7.5)
        if not self.datos_turno["mercancia"]:
            pdf.cell(190, 5.5, _limpiar_texto_pdf("No se registraron ventas de mercancia en este turno."), 1, 1, "C")
        else:
            for m in self.datos_turno["mercancia"]:
                metodos_prod = _limpiar_texto_pdf(self.formatear_metodos_str(m["usd"], m["bs"], m["p2p"], m["punto"]))
                pdf.cell(48, 5.5, _limpiar_texto_pdf(str(m["producto"])[:28]), 1, 0, "L")
                pdf.cell(16, 5.5, str(m["cantidad"]), 1, 0, "C")
                pdf.cell(66, 5.5, metodos_prod[:42], 1, 0, "C")
                pdf.cell(34, 5.5, _limpiar_texto_pdf(formatear_bs(m["total_bs"])), 1, 0, "R")
        ...
        if self.datos_turno["danos"]:
            ...
            # Anchos calculados al milímetro: 22 + 46 + 54 + 26 + 24 + 18 = 190 mm
            w_hab = 22
            w_huesped = 46
            w_concepto = 54
            w_metodo = 26
            ...
            # Encabezado con 'HABITACIÓN'
            pdf.set_fill_color(245, 230, 230)
            pdf.set_font("Helvetica", "B", 8)
            pdf.cell(w_hab, 6, "HABITACIÓN", 1, 0, "C", fill=True)
            pdf.cell(w_huesped, 6, "HUÉSPED RESPONSABLE", 1, 0, "L", fill=True)
            pdf.cell(w_concepto, 6, "CONCEPTO DEL DAÑO", 1, 0, "L", fill=True)
            pdf.cell(w_metodo, 6, "MÉTODO", 1, 0, "C", fill=True)
            pdf.cell(w_bs, 6, "MONTO (BS)", 1, 0, "R", fill=True)
            pdf.cell(w_usd, 6, "MONTO ($)", 1, 0, "R", fill=True)
            pdf.ln()
            ...
            for d in self.datos_turno["danos"]:
                ...
                # 3. Tratamiento para Recuperación de Deuda vs Daño normal
                raw_concepto = d["concepto_dano"]
                cod_hab = d["hab_codigo"]

                if "Recuperación Deuda" in raw_concepto or "Recuperacion Deuda" in raw_concepto or "RECUPERACIÓN" in cod_hab:
                    # Extraer el código de la habitación si venía en el motivo (ej. 'E-302')
                    match_hab = re.search(r"Hab\s+([A-Z0-9\-]+)", raw_concepto, re.IGNORECASE)
                    if match_hab:
                        hab_celda = match_hab.group(1).upper()
                    elif cod_hab not in ["RECUPERACIÓN_DEUDA", "RECUPERACION_DEUDA"]:
                        hab_celda = cod_hab
                    else:
                        hab_celda = "RECUP."
                    concepto_celda = "Recuperación de Deuda"
                else:
                    hab_celda = cod_hab
                    concepto_celda = raw_concepto

                # 4. Imprimir celdas perfectamente alineadas sin solapamiento
                pdf.cell(w_hab, 5.5, hab_celda[:12], 1, 0, "C")
                pdf.cell(w_huesped, 5.5, huesped_txt[:26], 1, 0, "L")
                pdf.cell(w_concepto, 5.5, concepto_celda[:32], 1, 0, "L")
                pdf.cell(w_metodo, 5.5, met_str[:15], 1, 0, "C")
                pdf.cell(w_bs, 5.5, formatear_bs(d["monto_bs"]).replace(" Bs", ""), 1, 0, "R")
                pdf.cell(w_usd, 5.5, f"${d['monto_usd']:.2f}", 1, 1, "R")
        ...
        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(190, 6, _limpiar_texto_pdf("CONCILIACION POR METODO DE PAGO"), 1, 1, "C", fill=True)

        pdf.set_font("Helvetica", "", 8.5)
        pdf.cell(130, 5, _limpiar_texto_pdf("  Total Efectivo Dolares ($):"), 1, 0, "L")
        pdf.cell(60, 5, f"${self.datos_turno['total_efectivo_usd']:.2f}", 1, 1, "R")

        pdf.cell(130, 5, _limpiar_texto_pdf("  Total Efectivo Bolivares:"), 1, 0, "L")
        pdf.cell(60, 5, _limpiar_texto_pdf(formatear_bs(self.datos_turno['total_efectivo_bs'])), 1, 1, "R")

        pdf.cell(130, 5, _limpiar_texto_pdf("  Total Pago Movil (Bs):"), 1, 0, "L")
        pdf.cell(60, 5, _limpiar_texto_pdf(formatear_bs(self.datos_turno['total_p2p_bs'])), 1, 1, "R")

        pdf.cell(130, 5, _limpiar_texto_pdf("  Total Punto de Venta (Bs):"), 1, 0, "L")
        pdf.cell(60, 5, _limpiar_texto_pdf(formatear_bs(self.datos_turno['total_punto_bs'])), 1, 1, "R")
        pdf.ln(4)

        # Totales Generales
        pdf.set_fill_color(220, 230, 242)
        pdf.set_font("Helvetica", "B", 9.5)
        pdf.cell(130, 7, _limpiar_texto_pdf("TOTAL GENERAL EN BOLIVARES (BS) RECAUDADOS:"), 1, 0, "L", fill=True)
        pdf.set_font("Helvetica", "B", 10.5)
        pdf.cell(60, 7, _limpiar_texto_pdf(formatear_bs(self.datos_turno['total_bs_consolidado'])), 1, 1, "R", fill=True)

        pdf.set_fill_color(220, 242, 225)
        pdf.set_font("Helvetica", "B", 9.5)
        pdf.cell(130, 7, _limpiar_texto_pdf("TOTAL GENERAL EN DIVISAS ($) EN EFECTIVO:"), 1, 0, "L", fill=True)
        pdf.set_font("Helvetica", "B", 10.5)
        ...
        pdf.cell(60, 4, _limpiar_texto_pdf("ENTREGADO POR"), 0, 0, "C")

        pdf.line(130, pos_y, 190, pos_y)
        pdf.set_xy(130, pos_y + 1)
        pdf.cell(60, 4, _limpiar_texto_pdf("RECIBIDO POR (ADMINISTRACION)"), 0, 0, "C")
