...
def enviar_habitacion_mantenimiento(hab_codigo: str, tipo_averia: str, descripcion: str, tecnico: str, costo_usd: float, user: str) -> bool:
    """Bloquea la habitación a estado 'Mantenimiento' usando la fecha y hora local del equipo."""
    conn = obtener_conexion()
    cursor = conn.cursor()
    tasa = obtener_tasa_bcv()
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO mantenimiento_habitaciones (
                ...
                costo_reparacion_usd, costo_reparacion_bs, fecha_inicio, estado, reportado_por
            ) VALUES (?, ?, ?, ?, ?, ?, ?, 'EN_PROCESO', ?);
        """, (hab_codigo, tipo_averia, descripcion, tecnico, costo_usd, costo_usd * tasa, ahora_local, user))
...
def liquidar_deuda_moroso_y_rehabilitar(cedula: str, nombre: str, motivo_original: str, monto_usd: float, metodo: str, user: str, p2p_ref=None, p2p_ci=None, p2p_tel=None) -> bool:
    ...
    turno = "MAÑANA" if 8 <= ahora.hour < 17 else "NOCHE"
    import re
    match = re.search(r"Hab\s+([A-Z0-9\-]+)", motivo_original, re.IGNORECASE)
    hab_ref = f" ({match.group(0)})" if match else ""
    concepto = f"Recuperación Deuda{hab_ref}"
    try:
        conn.execute("BEGIN TRANSACTION;")
