...
from database.db_manager import (
    obtener_historial_saime,
    obtener_reporte_p2p,
    obtener_reporte_utilidad_inventario,
    obtener_historial_limpieza,
    obtener_tasa_bcv,
    formatear_bs,
    formatear_usd,
...
class PDFPlanillaSAIME(FPDF):
    ...
    def header(self):
        ...
        if os.path.exists(logo_h):
            self.image(logo_h, x=10, y=6, w=18)
        if os.path.exists(logo_s):
            self.image(logo_s, x=245, y=3, w=35)
        ...
        self.cell(
            0, 4.5, "DIRECCIÓN GENERAL DE MIGRACIÓN Y EXTRANJERÍA - SAIME", 0, 1, "C"
        )
        self.set_font("Helvetica", "B", 11)
        self.cell(
            0,
            5,
            "INVERSIONES SAIBABA C.A. - MOTEL EL EDEN | RIF: J-30250227-6",
            0,
            1,
            "C",
        )
        ...
        self.cell(
            0, 4, "REGISTRO DIARIO DE HUÉSPEDES Y CONTROL DE EXTRANJERÍA", 0, 1, "C"
        )
        self.set_y(32)
    ...
    def footer(self):
        ...
        self.cell(
            0,
            8,
            f"Planilla Oficial para Inspección Policial / SAIME - Página {self.page_no()}",
            0,
            0,
            "C",
        )
...
class PDFReporteP2P(FPDF):
    ...
    def header(self):
        ...
        self.cell(
            0,
            4.5,
            "RIF: J-30250227-6 | REPORTE DE AUDITORÍA DE TRANSACCIONES PAGO MÓVIL (P2P)",
            0,
            1,
            "C",
        )
    ...
    def footer(self):
        ...
        self.cell(
            0,
            8,
            f"Auditoría Contable Bancaria P2P - Página {self.page_no()}",
            0,
            0,
            "C",
        )
...
class PDFReporteUtilidadCPP(FPDF):
    ...
    def header(self):
        ...
        self.cell(
            0,
            4.5,
            "RIF: J-30250227-6 | ESTADO DE RESULTADOS: UTILIDAD Y GANANCIAS MINI-BAR (CPP)",
            0,
            1,
            "C",
        )
    ...
    def footer(self):
        ...
        self.cell(
            0,
            8,
            f"Auditoría Contable de Rentabilidad - Página {self.page_no()}",
            0,
            0,
            "C",
        )
...
class PDFReporteLimpieza(FPDF):
    ...
    def header(self):
        ...
        self.cell(
            0,
            4.5,
            "RIF: J-30250227-6 | REGISTRO DE LIMPIEZAS Y CONTROL DE CAMARERAS (SLA)",
            0,
            1,
            "C",
        )
    ...
    def footer(self):
        ...
        self.cell(
            0,
            8,
            f"Auditoría de Camareras y Housekeeping - Página {self.page_no()}",
            0,
            0,
            "C",
        )
...
class FrameReportes(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(
            f_title_box,
            text="CENTRO DE REPORTES Y AUDITORÍA",
        ...
        ctk.CTkLabel(
            f_title_box,
            text="Libros oficiales SAIME, transacciones P2P, rentabilidad CPP y control SLA",
            font=("Arial", 9),
        ...
        self.tab_selector = ctk.CTkSegmentedButton(
            ...
            selected_color=self.C_GOLD,
            selected_hover_color=self.C_GOLD_HOVER,
            unselected_color=self.C_CARD,
            unselected_hover_color="#DF6721",
            text_color="#F7F6F3",
            font=("Arial", 11, "bold"),
        ...
        self.entry_search = ctk.CTkEntry(
            f_row2,
            placeholder_text="🔍 Filtrar registros en pantalla...",
            width=260,
            height=32,
    ...
    def cargar_datos(self):
        ...
        if "SAIME" in tipo:
            ...
            prom_sla = (
                (
                    sum(l["duracion_minutos"] for l in self.datos_limpieza)
                    / len(self.datos_limpieza)
                )
                if self.datos_limpieza
                else 0
            )
            self.lbl_resumen_kpi.configure(
    ...
    def filtrar_pantalla(self):
        ...
        if "SAIME" in tipo:
            if not self.datos_saime:
                self._render_vacio(
                    "No hay registros de extranjería / SAIME en el rango de fechas seleccionado."
                )
                return
            ...
            for est in self.datos_saime:
                ...
                tit_ci = est["cedula"]
                ac_nom = (
                    f"{est['ac_nombre']} {est['ac_apellido']}".upper()
                    if est.get("ac_nombre")
                    else ""
                )
                ...
                if query and not any(
                    query in campo for campo in [tit_nom, tit_ci, ac_nom, ac_ci, hab]
                ):
                    nro += 2 if ac_nom.strip() else 1
                    continue

                fecha_f = datetime.strptime(
                    est["fecha_entrada"], "%Y-%m-%d %H:%M:%S"
                ...
                ctk.CTkLabel(
                    f_t_izq,
                    text=f"#{nro:04d}  •  {tit_nom}  (C.I: {tit_ci})",
                    font=("Montserrat", 11, "bold"),
                    text_color=self.C_TEXT_MAIN,
                ).pack(anchor="w")
                ctk.CTkLabel(
                    f_t_izq,
                    text=f"Fecha: {fecha_f}  |  Hab: {hab}  |  Edad: {est['edad']} años  |  Ruta: {est['procedencia']} ➔ {est['destino']}",
                    font=("Arial", 9),
                    text_color=self.C_TEXT_MUTED,
                ).pack(anchor="w")

                badge_t = ctk.CTkFrame(
                    card_t,
                    fg_color="#211E1B",
                    corner_radius=5,
                    border_width=1,
                ...
                if (
                    est.get("ac_nombre")
                    and est.get("ac_cedula")
                    and est["ac_nombre"].strip()
                ):
                    card_a = ctk.CTkFrame(
                        self.scroll_data,
                        fg_color="#181614",
                    ...
                    ctk.CTkLabel(
                        f_a_izq,
                        text=f"#{nro:04d}  •  {ac_nom}  (C.I: {ac_ci})",
                        font=("Montserrat", 11, "bold"),
                        text_color=self.C_TEXT_MAIN,
                    ).pack(anchor="w")
                    ctk.CTkLabel(
                        f_a_izq,
                        text=f"Fecha: {fecha_f}  |  Hab: {hab}  |  Edad: {est['ac_edad']} años  |  Ruta: {est['ac_procedencia']} ➔ {est['ac_destino']}",
                        font=("Arial", 9),
                        text_color=self.C_TEXT_MUTED,
                    ).pack(anchor="w")

                    badge_a = ctk.CTkFrame(
                        card_a,
                        fg_color="#161513",
                        corner_radius=5,
                        border_width=1,
                        border_color=self.C_TEXT_MUTED,
                    )
            ...
            if not self.datos_p2p:
                self._render_vacio(
                    "No se registraron cobros con Pago Móvil en este período."
                )
                return
            ...
            for p in self.datos_p2p:
                ...
                if query and not any(
                    query in campo for campo in [cli, ref, ci, hab, con]
                ):
                    continue
                ...
                ctk.CTkLabel(
                    f_izq,
                    text=f"Hab {p['hab_codigo']}  •  {p['concepto']}  •  {p['cliente_nombre']}",
                    font=("Montserrat", 11, "bold"),
                    text_color=self.C_TEXT_MAIN,
                ).pack(anchor="w")
                ctk.CTkLabel(
                    f_izq,
                    text=f"Fecha: {p['fecha']}  |  Ref: {p['p2p_referencia']}  |  C.I: {p['p2p_ci']}  |  Tel: {p['p2p_telefono']}  |  Cajero: {p['recepcionista']}",
                    font=("Arial", 9),
                    text_color=self.C_TEXT_MUTED,
                ).pack(anchor="w")
                ...
                ctk.CTkLabel(
                    f_der,
                    text=f"${p['monto_usd']:.2f}",
                    font=("Montserrat", 13, "bold"),
                    text_color=self.C_GOLD,
                ).pack(anchor="e")
                ctk.CTkLabel(
                    f_der,
                    text=formatear_bs(p["monto_bs"]),
                    font=("Arial", 9, "bold"),
                    text_color=self.C_GREEN,
                ).pack(anchor="e")
            ...
            if not self.datos_cpp:
                self._render_vacio(
                    "No hay ventas registradas de mini-bar para calcular utilidad."
                )
                return
            ...
            for d in self.datos_cpp:
                ...
                ctk.CTkLabel(
                    f_izq,
                    text=f"Vendidos: {d['unidades_vendidas']} un.  |  Venta Total: ${d['ingreso_total_usd']:.2f}  |  Costo Total (CPP): ${d['costo_total_usd']:.2f}",
                    font=("Arial", 9),
                    text_color=self.C_TEXT_MUTED,
                ).pack(anchor="w")
                ...
                ctk.CTkLabel(
                    f_der,
                    text=f"+${d['utilidad_neta_usd']:.2f} ({d['margen_pct']:.1f}%)",
                    font=("Montserrat", 12, "bold"),
                    text_color=self.C_GREEN,
                ).pack(anchor="e")
                ctk.CTkLabel(
                    f_der,
                    text=formatear_bs(d["utilidad_neta_bs"]),
                    font=("Arial", 9),
                    text_color=self.C_TEXT_MUTED,
                ).pack(anchor="e")
            ...
            if not self.datos_limpieza:
                self._render_vacio(
                    "No hay registros de limpieza en el período seleccionado."
                )
                return
            ...
            for lim in self.datos_limpieza:
                ...
                ctk.CTkLabel(
                    f_izq,
                    text=f"Habitación {lim['hab_codigo']}  •  Camarera: {lim['camarera']}",
                    font=("Montserrat", 11, "bold"),
                    text_color=self.C_TEXT_MAIN,
                ).pack(anchor="w")
                ctk.CTkLabel(
                    f_izq,
                    text=f"Liberada: {lim['fecha_limpia']}  |  Pasó a Sucia: {lim['fecha_sucia'] or 'N/A'}  |  Turno: {lim['turno']}",
                    font=("Arial", 9),
                    text_color=self.C_TEXT_MUTED,
                ).pack(anchor="w")
    ...
    def exportar_pdf_actual(self):
        ...
        if "SAIME" in t:
            self.exportar_saime_pdf()
        elif "P2P" in t:
            self.exportar_p2p_pdf()
        elif "Utilidad" in t:
            self.exportar_cpp_pdf()
        else:
            self.exportar_limpieza_pdf()

    def exportar_excel_actual(self):
        t = self.tab_selector.get()
        if "SAIME" in t:
            self.exportar_saime_excel()
        elif "P2P" in t:
            self.exportar_p2p_excel()
        elif "Utilidad" in t:
            self.exportar_cpp_excel()
        else:
            self.exportar_limpieza_excel()
    ...
    def exportar_saime_pdf(self):
        ...
        for est in self.datos_saime:
            ...
            if (
                ...
                pdf.cell(22, 6, fecha_f, 1, 0, "C")
                pdf.cell(
                    65, 6, f"{est['ac_nombre']} {est['ac_apellido']}".upper()[:35], 1
                )
                ...
                pdf.cell(
                    ...
                    6,
                    (
                        str(est["ac_estado_civil"])[0].upper()
                        if est.get("ac_estado_civil")
                        else "S"
                    ),
                    1,
                    0,
                ...
                pdf.cell(
                    ...
                    6,
                    (
                        str(est["ac_nacionalidad"]).upper()
                        if est.get("ac_nacionalidad")
                        else "V"
                    ),
                    1,
                    0,
        ...
        ruta = self.obtener_ruta_exportacion(
            f"planilla_saime_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        )
        pdf.output(ruta)
    ...
    def exportar_saime_excel(self):
        ...
        ws = wb.active
        ws.title = "Planilla SAIME"
        ws.views.sheetView[0].showGridLines = True

        # -------------------------------------------------------------------------
        # 1. ESTILOS Y PALETA DE COLORES CORPORATIVOS
        # -------------------------------------------------------------------------
        f_titulo_pais = Font(name="Calibri", size=9, bold=True, color="1C1A17")
        f_sub_gob = Font(name="Calibri", size=8, bold=True, color="444444")
        f_empresa = Font(
            name="Calibri", size=11, bold=True, color="D4AF37"
        )  # Dorado Champagne
        f_emision = Font(name="Calibri", size=8, italic=True, color="666666")
        f_header = Font(name="Calibri", size=9, bold=True, color="FFFFFF")
        f_data = Font(name="Calibri", size=9, color="111111")
        f_tag_acomp = Font(name="Calibri", size=8, italic=True, color="555555")

        fill_header = PatternFill(
            start_color="211E1B", end_color="211E1B", fill_type="solid"
        )
        fill_zebra = PatternFill(
            start_color="F9F8F6", end_color="F9F8F6", fill_type="solid"
        )
        ...
        # -------------------------------------------------------------------------
        # 2. ENCABEZADO OFICIAL SAIME (IGUAL AL PDF)
        # -------------------------------------------------------------------------
        lineas_encabezado = [
            ("C1:G1", "REPÚBLICA BOLIVARIANA DE VENEZUELA", f_titulo_pais, 16),
            (
                "C2:G2",
                "MINISTERIO DEL PODER POPULAR PARA RELACIONES INTERIORES, JUSTICIA Y PAZ",
                f_sub_gob,
            ...
            (
                "C3:G3",
                "DIRECCIÓN GENERAL DE MIGRACIÓN Y EXTRANJERÍA - SAIME",
                f_sub_gob,
                14,
            ),
            (
            ...
            (
                "C5:G5",
                f"REGISTRO DIARIO DE HUÉSPEDES  •  EMISIÓN: {datetime.now().strftime('%d/%m/%Y %I:%M %p')}",
                f_emision,
                14,
            ),
        ]

        for rango, texto, fuente, alto in lineas_encabezado:
            ws.merge_cells(rango)
            celda_ref = ws[rango.split(":")[0]]
            celda_ref.value = texto
            celda_ref.font = fuente
            celda_ref.alignment = align_center
            fila_num = int(rango.split(":")[0][1:])
            ws.row_dimensions[fila_num].height = alto

        # Fila 6: Separador estético
        ws.row_dimensions[6].height = 6

        # Logos institucionales (Hotel a la izquierda, SAIME a la derecha)
        ruta_logo_hotel = obtener_ruta_recurso("logo_hotel.png")
        if os.path.exists(ruta_logo_hotel):
            try:
                img_h = OpenpyxlImage(ruta_logo_hotel)
                img_h.width = 75
                img_h.height = 75
                ws.add_image(img_h, "A1")
            except Exception:
                pass

        ruta_logo_saime = obtener_ruta_recurso("logo_saime.png")
        if os.path.exists(ruta_logo_saime):
            try:
                img_s = OpenpyxlImage(ruta_logo_saime)
                img_s.width = 90
                img_s.height = 65
                ws.add_image(img_s, "I1")
            except Exception:
                pass

        # -------------------------------------------------------------------------
        # 3. CABECERA DE LA TABLA (FILA 7)
        # -------------------------------------------------------------------------
        headers = [
            "N° PER",
            "FECHA",
        ...
        # -------------------------------------------------------------------------
        # 4. CARGA DE HUÉSPEDES (TITULARES Y ACOMPAÑANTES)
        # -------------------------------------------------------------------------
        nro = 1
        row_idx = 8
        ...
        for est in self.datos_saime:
            try:
                ...
                fecha_f = str(est["fecha_entrada"])[:10]

            # Fila Titular
            fila_t = [
                nro,
                fecha_f,
                f"{est['nombre']} {est['apellido']}".upper(),
                int(est["edad"]) if str(est["edad"]).isdigit() else est["edad"],
                str(est["estado_civil"])[0].upper(),
                str(est["nacionalidad"]).upper(),
                str(est["procedencia"]).upper(),
                str(est["cedula"]),
                str(est["destino"]).upper(),
            ]

            ws.row_dimensions[row_idx].height = 20
            fill_actual = fill_white if (nro % 2 != 0) else fill_zebra

            for c_idx, val in enumerate(fila_t, start=1):
                cell = ws.cell(row=row_idx, column=c_idx, value=val)
                cell.font = f_data
                cell.fill = fill_actual
                cell.border = border_cell
                cell.alignment = align_left if c_idx in [3, 7, 9] else align_center

            nro += 1
            row_idx += 1

            # Fila Acompañante (si aplica)
            if (
                est.get("ac_nombre")
                and est.get("ac_cedula")
                and est["ac_nombre"].strip()
                ...
                fila_a = [
                    ...
                    (
                        int(est["ac_edad"])
                        if str(est.get("ac_edad", "")).isdigit()
                        else est.get("ac_edad", "")
                    ),
                    (
                        str(est.get("ac_estado_civil", "S"))[0].upper()
                        if est.get("ac_estado_civil")
                        else "S"
                    ),
                    str(est.get("ac_nacionalidad", "V")).upper(),
                    str(est.get("ac_procedencia", "")).upper(),
                    str(est["ac_cedula"]),
                    str(est.get("ac_destino", "")).upper(),
                ]

                ws.row_dimensions[row_idx].height = 20
                fill_actual = fill_white if (nro % 2 != 0) else fill_zebra

                for c_idx, val in enumerate(fila_a, start=1):
                    cell = ws.cell(row=row_idx, column=c_idx, value=val)
                    cell.font = f_data
                    cell.fill = fill_actual
                    cell.border = border_cell
        ...
        # -------------------------------------------------------------------------
        # 5. ANCHOS DE COLUMNA Y GUARDADO SEGURO
        # -------------------------------------------------------------------------
        anchos = {
            "A": 10,
            "B": 14,
        ...
        ruta = self.obtener_ruta_exportacion(
            f"planilla_saime_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        )
        wb.save(ruta)
        os.system(f'start "" "{ruta}"' if os.name == "nt" else f'open "{ruta}"')
    ...
    def exportar_p2p_pdf(self):
        ...
        ruta = self.obtener_ruta_exportacion(
            f"reporte_p2p_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        )
        pdf.output(ruta)
        os.system(f'start "" "{ruta}"' if os.name == "nt" else f'open "{ruta}"')
    ...
    def exportar_p2p_excel(self):
        ...
        ruta = self.obtener_ruta_exportacion(
            f"auditoria_p2p_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        )
        wb.save(ruta)
        os.system(f'start "" "{ruta}"' if os.name == "nt" else f'open "{ruta}"')
    ...
    def exportar_cpp_pdf(self):
        ...
        margen_global_pct = (
            (tot_util_usd / tot_ing_usd * 100) if tot_ing_usd > 0 else 0.0
        )
        ...
        ruta = self.obtener_ruta_exportacion(
            f"utilidad_minibar_cpp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        )
        pdf.output(ruta)
        os.system(f'start "" "{ruta}"' if os.name == "nt" else f'open "{ruta}"')
    ...
    def exportar_cpp_excel(self):
        ...
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = "Utilidad Mini-Bar CPP"
        ws.append(
            [
                "Producto",
                "Cant",
        ...
        ruta = self.obtener_ruta_exportacion(
            f"utilidad_minibar_cpp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        )
        wb.save(ruta)
        os.system(f'start "" "{ruta}"' if os.name == "nt" else f'open "{ruta}"')
    ...
    def exportar_limpieza_pdf(self):
        if not self.datos_limpieza:
            messagebox.showwarning(
                "Atención", "No hay registros de limpieza para exportar."
            )
            return
        ...
        for l in self.datos_limpieza:
            ...
            pdf.cell(
                50,
                6,
                str(l["fecha_sucia"])[:16] if l["fecha_sucia"] else "N/A",
                1,
                0,
                "C",
            )
        ...
        ruta = self.obtener_ruta_exportacion(
            f"control_camareras_sla_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        )
        pdf.output(ruta)
        os.system(f'start "" "{ruta}"' if os.name == "nt" else f'open "{ruta}"')
    ...
    def exportar_limpieza_excel(self):
        ...
        ruta = self.obtener_ruta_exportacion(
            f"control_camareras_sla_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        )
        wb.save(ruta)
        os.system(f'start "" "{ruta}"' if os.name == "nt" else f'open "{ruta}"')
