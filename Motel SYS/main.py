...
class Application(ctk.CTk):
    ...
    def mostrar_login(self):
        if self.usuario_actual:
            try:
                registrar_auditoria(
                    ...
                    "LOGOUT", "Cierre de sesión regular del usuario."
                )
            except Exception:
                pass
            self.usuario_actual = None
        ...
        # Canvas decorativo de fondo centrado
        ...
        card_login = ctk.CTkFrame(
            center_box, width=440, height=520,
            fg_color=self.C_BG_SIDEBAR, corner_radius=16,
            border_width=1.5, border_color=self.C_BORDER
        )
        card_login.pack()
        card_login.pack_propagate(False)

        # Logotipo / Escudo Simbólico
        f_badge = ctk.CTkFrame(card_login, width=54, height=54, fg_color="#201C16", corner_radius=27, border_width=1, border_color=self.C_GOLD)
        f_badge.pack(pady=(35, 10))
        f_badge.pack_propagate(False)
        ctk.CTkLabel(f_badge, text="⚜", font=("Georgia", 22), text_color=self.C_GOLD).place(relx=0.5, rely=0.5, anchor="center")

        # Títulos
        ctk.CTkLabel(card_login, text="SISTEMA DE GESTIÓN", font=("Montserrat", 20, "bold"), text_color=self.C_GOLD).pack(pady=(2, 0))
        ctk.CTkLabel(card_login, text="CONTROL HOTELERO & MOTELES", font=("Montserrat", 10, "bold"), text_color=self.C_TEXT_MUTED).pack(pady=(0, 25))

        # Campos de texto estilizados
        f_inputs = ctk.CTkFrame(card_login, fg_color="transparent")
        f_inputs.pack(fill="x", padx=45)

        ctk.CTkLabel(f_inputs, text="USUARIO DEL SISTEMA", font=("Arial", 9, "bold"), text_color=self.C_TEXT_MUTED).pack(anchor="w", pady=(0, 3))
        e_user = ctk.CTkEntry(
            f_inputs, placeholder_text="Nombre de usuario", height=42,
            corner_radius=8, fg_color=self.C_CARD, border_color=self.C_BORDER,
            text_color=self.C_TEXT_MAIN, font=("Arial", 11)
        )
        e_user.pack(fill="x", pady=(0, 14))
        e_user.focus_set()

        ctk.CTkLabel(f_inputs, text="CONTRASEÑA DE ACCESO", font=("Arial", 9, "bold"), text_color=self.C_TEXT_MUTED).pack(anchor="w", pady=(0, 3))
        e_pass = ctk.CTkEntry(
            f_inputs, placeholder_text="••••••••••••", show="*", height=42,
            corner_radius=8, fg_color=self.C_CARD, border_color=self.C_BORDER,
            text_color=self.C_TEXT_MAIN, font=("Arial", 11)
        )
        e_pass.pack(fill="x", pady=(0, 18))

        lbl_error = ctk.CTkLabel(f_inputs, text="", font=("Arial", 10, "bold"), text_color="#E74C3C")
        ...
        def intentar_login(event=None):
            ...
            if user_data:
                ...
                lbl_error.configure(text="❌ Credenciales incorrectas o usuario inactivo.")
                e_pass.delete(0, 'end')
        ...
        btn_login = ctk.CTkButton(
            card_login, text="Iniciar Turno / Sesión  ➔", height=44,
            corner_radius=8, fg_color=self.C_GOLD, hover_color=self.C_GOLD_HOVER,
            text_color="#100F0D", font=("Montserrat", 12, "bold"),
            command=intentar_login
        )
        btn_login.pack(fill="x", padx=45, pady=(5, 15))

        # Indicador inferior de motor seguro
        ctk.CTkLabel(card_login, text="● BASE DE DATOS LOCAL CIFRADA Y PROTEGIDA", font=("Arial", 8, "bold"), text_color="#3E8E5A").pack(side="bottom", pady=12)
    ...
    def construir_dashboard(self):
        ...
        self.sidebar = ctk.CTkFrame(
            self.main_container, width=270, fg_color=self.C_BG_SIDEBAR,
            corner_radius=0, border_width=1, border_color=self.C_BORDER
        )
        ...
        self.content_area = ctk.CTkFrame(
            self.main_container, fg_color=self.C_BG_CONTENT,
            corner_radius=0
        )
        ...
        ctk.CTkLabel(f_brand, text="HOTEL & SUITES", font=("Montserrat", 15, "bold"), text_color=self.C_GOLD).pack(anchor="w")
        ctk.CTkLabel(f_brand, text="Sistema Integral de Control", font=("Arial", 9), text_color=self.C_TEXT_MUTED).pack(anchor="w")

        # Tarjeta de Usuario + Turno + Reloj
        card_session = ctk.CTkFrame(self.sidebar, fg_color=self.C_CARD, corner_radius=10, border_width=1, border_color=self.C_BORDER)
        ...
        ctk.CTkLabel(f_user_row, text=f"👤 {self.usuario_actual['usuario']}", font=("Montserrat", 11, "bold"), text_color=self.C_TEXT_MAIN).pack(side="left")
        
        rol_tag = self.usuario_actual['rol'].upper()
        col_rol = "#E67E22" if rol_tag == "ADMIN" else "#2ECC71"
        b_rol = ctk.CTkFrame(f_user_row, fg_color="#181614", corner_radius=5, border_width=1, border_color=col_rol)
        b_rol.pack(side="right")
        ctk.CTkLabel(b_rol, text=rol_tag, font=("Arial", 8, "bold"), text_color=col_rol).pack(padx=5, pady=1)

        # Reloj Dinámico con Fecha
        self.lbl_fecha = ctk.CTkLabel(card_session, text="", font=("Arial", 9), text_color=self.C_TEXT_MUTED)
        self.lbl_fecha.pack(anchor="w", padx=12, pady=(0, 1))

        self.lbl_reloj = ctk.CTkLabel(card_session, text="", font=("Montserrat", 14, "bold"), text_color=self.C_GOLD)
        self.lbl_reloj.pack(anchor="w", padx=12, pady=(0, 2))

        # Badge de Turno
        self.lbl_turno = ctk.CTkLabel(card_session, text="", font=("Arial", 9, "bold"), text_color="#3498DB")
        ...
        def crear_boton_menu(texto, frame_class, icon):
            btn = ctk.CTkButton(
                self.menu_scroll, text=f"  {icon}   {texto}", anchor="w",
                height=38, corner_radius=8, fg_color="transparent",
                hover_color="#26221D", text_color=self.C_TEXT_MAIN,
                font=("Arial", 11, "bold"),
        ...
        # Sección Operativa
        ctk.CTkLabel(self.menu_scroll, text="OPERACIONES PRINCIPALES", font=("Montserrat", 8, "bold"), text_color=self.C_TEXT_MUTED).pack(anchor="w", padx=14, pady=(8, 4))
        ...
        ctk.CTkLabel(self.menu_scroll, text="AUDITORÍA & GERENCIA", font=("Montserrat", 8, "bold"), text_color=self.C_TEXT_MUTED).pack(anchor="w", padx=14, pady=(14, 4))
        ...
        if self.usuario_actual["rol"] == "admin":
            ...
            ctk.CTkLabel(self.menu_scroll, text="SISTEMA & DATOS", font=("Montserrat", 8, "bold"), text_color=self.C_TEXT_MUTED).pack(anchor="w", padx=14, pady=(14, 4))

            ctk.CTkButton(
                self.menu_scroll, text="  💾   Respaldar Base Datos", anchor="w",
                height=34, corner_radius=8, fg_color="#211E1B", hover_color="#2F2A25",
                text_color=self.C_GOLD, font=("Arial", 10, "bold"),
                border_width=1, border_color=self.C_BORDER,
                command=self.crear_respaldo_bd
            ).pack(fill="x", padx=8, pady=3)

            ctk.CTkButton(
                self.menu_scroll, text="  ⚠️   Resetear Fábrica", anchor="w",
                height=34, corner_radius=8, fg_color="#2D1715", hover_color="#451E1A",
                text_color="#F1948A", font=("Arial", 10, "bold"),
                border_width=1, border_color="#5B2C27",
        ...
        ctk.CTkButton(
            f_bottom, text="🚪  Cerrar Sesión", height=38, corner_radius=8,
            fg_color="#24201C", hover_color="#78281F", text_color=self.C_TEXT_MAIN,
    ...
    def navegar_a(self, FrameClass, texto_btn):
        ...
        for t, btn in self.nav_buttons.items():
            if t == texto_btn:
                btn.configure(
                    fg_color=self.C_GOLD, text_color="#100F0D",
                    hover_color=self.C_GOLD_HOVER
                )
            else:
                btn.configure(
                    fg_color="transparent", text_color=self.C_TEXT_MAIN,
                    hover_color="#26221D"
                )
    ...
    def actualizar_reloj_y_turno(self):
        try:
            if hasattr(self, 'lbl_reloj') and self.lbl_reloj.winfo_exists():
                ...
                fecha_espanol = f"{nombre_dia}, {ahora.day:02d} {nombre_mes} {ahora.year}"
    ...
    def crear_respaldo_bd(self):
        ...
        nombre_sugerido = f"Respaldo_Motel_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
        ...
        ruta_destino = filedialog.asksaveasfilename(
            ...
            defaultextension=".db",
            filetypes=[("Base de Datos SQLite (*.db)", "*.db"), ("Todos los archivos", "*.*")]
        )
        ...
        try:
            ...
            if generar_respaldo_seguro(ruta_destino):
                registrar_auditoria(
                    usr_act, rol_act, "CREAR_RESPALDO",
            ...
            messagebox.showerror("Error al Respaldar", f"Ocurrió un error inesperado:\n{e}")
    ...
    def ejecutar_reseteo_bd(self):
        ...
        if confirmar:
            registrar_auditoria(usr_act, rol_act, "RESETEO_BD", "Restablecimiento de fábrica ejecutado por Administrador.")
            if resetear_base_datos():
                messagebox.showinfo("Sistema Restaurado", "La base de datos fue reinicializada con los valores por defecto.")
                self.mostrar_login()
            else:
                messagebox.showerror("Error", "Ocurrió una falla al intentar resetear la base de datos.")
